...
/*函数节流
* var a=throttle(function(){执行方法},1000)
*  a()放到事件里
*
* **********************/
...
export function dateformats(date,fmt){
    ...
    var o = {
        ...
        "m+": te.getMinutes(), //分
        "s+": te.getSeconds(), //秒
        "q+": Math.floor((te.getMonth() + 3) / 3), //季度
        "S": te.getMilliseconds() //毫秒
    };
    if (/(y+)/.test(fmt)) fmt = fmt.replace(RegExp.$1, (te.getFullYear() + "").substr(4 - RegExp.$1.length));
    for (var k in o)
...
export const encryption = (params) => {
    ...
    if (type === 'Base64') {
        ...
        param.forEach(ele => {
            var data = result[ele]
            key = window.CryptoJS.enc.Latin1.parse(key)
            var iv = key
                // 加密
...
export const setTitle = function(title) {
        title = title ? `${title}——Bref 微服务快速开发框架` : 'Bref 微服务快速开发框架'
...
export const sortModule = (arr) => {
    ...
    for (i=0; i<len; i++) {
        ...
        if(arr[i].children){
            arr[i].children=sortModule(arr[i].children);
        }
    }
    return arr;
};
