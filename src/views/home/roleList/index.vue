        ...
        <template slot="header">
            <el-form inline :model="search" class="form_search" @keyup.enter.native="searchSubmit">
                <el-form-item label="名称">
                    <el-input v-model="search.searchParam.name" :maxlength="50" placeholder="数据权限-列名称"></el-input>
                </el-form-item>
                <el-form-item label="标识">
                    <el-input v-model="search.searchParam.entry" :maxlength="50" placeholder="标识"></el-input>
                </el-form-item>
...
<script>
    import {getDataSchema,putDataSchema,addDataSchema,delDataSchema} from "@/api/roleList";
    ...
    export default {
        ...
        methods: {
            getlist(){
                getDataSchema(this.search).then(response => {
                    if(response.success){
            ...
            upsubmit (formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        //获取点击的节点权限(包含半节点)
                        (this.form.id?putDataSchema:addDataSchema)(this.form).then(res =>{
                            if(res.success){
            ...
            changeFun (row) {
                if(this.msg){
                    ...
                    this.$router.push({ path:'/roleList/info/'+row.id,query:{name:row.name}});
                }
