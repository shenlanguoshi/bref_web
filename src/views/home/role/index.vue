<template>
    <d2-container >
        <template slot="header">
            <el-form inline :model="search" class="form_search" @keyup.enter.native="searchSubmit">
                <el-form-item label="权限名">
                    <el-input v-model="search.searchParam.name" :maxlength="50" placeholder="请输入权限名"></el-input>
                </el-form-item>
                <el-form-item>
        ...
        <el-table class="main_table"
            ...
            <el-table-column prop="name" label="权限名称" align="center" min-width="200"></el-table-column>
...
<script>
    import {getPermission,putPermission,addPermission,delPermission} from "@/api/role";
    ...
    export default {
        ...
        methods: {
            getlist(){
                getPermission(this.search).then(response => {
                    if(response.success){
            ...
            upsubmit (formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        //获取点击的节点权限(包含半节点)
                        (this.form.id?putPermission:addPermission)(this.form).then(res =>{
                            if(res.success){
                                this.dialogFormVisible = false;
                                //重新获取列表页
                                this.getlist();
            ...
            changeFun (row ) {
                if(this.msg){
                    ...
                    this.$router.push({ path:'/role/info/'+row.id,query:{name:row.name}});
                }
            },
            rowCla ({row, rowIndex}) {
                return 'rowCla';
