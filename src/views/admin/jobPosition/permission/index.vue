...
<script>
    ...
    import {getJobPositionP,delJobPositionP,addJobPositionP} from "@/api/jobPosition";
    import waves from "@/directive/waves/index.js"; // 水波纹指令
    export default {
        name: "jobPositionPermission",
        directives: {
            waves
        },
        components:{
        ...
        methods: {
            getList () {
                ...
                getJobPositionP(this.id).then(response => {
            ...
            deletes (row){
                this.$confirm("此操作将永久删除该职务的权限(权限名:" + row.name + "), 是否继续?", '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    delJobPositionP({id:this.id,permissionId:row.id}).then((response) => {
                        if(response.data.success){
                            this.getList();
