...
<script>
    import {getIndustryCategory} from "@/api/industryCategory";
    export default {
        name: "tree",
        ...
        computed: {
            isShow: function () {
                // `this` 指向 vm 实例
                return Boolean(this.disabled) || Boolean(this.msg && this.msg.status) || Boolean(this.msg && this.msg.check)
            }
        },
        watch :{
            'disabled' (to) {
        ...
        methods: {
            getList () {
                getIndustryCategory().then(response => {
            ...
            checkedf () {
                if(this.msg.chs){
                    this.checked=this.msg.chs.split(',')
                }
            },
            ...
            getNodeData (data) {
                ...
                }else if(this.msg && this.msg.check){
                        this.$emit('node-click',{k:this.$refs.tree.getCheckedKeys(),d:this.$refs.tree.getCheckedNodes()});
                        //this.$refs.tree.setChecked(data,false);
                }else{
                    this.$emit('node-click',data);
                }
